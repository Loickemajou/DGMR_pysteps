...
@postprocess_import()
def read(data_folder,start_time_frame):
    ...
    for i, filename in enumerate(os.listdir(data_folder)):
      ...
      if filename.endswith('.hdf') and start_time_frame==time_str:
          fns=[f"{data_folder}/{file_name}" for file_name in os.listdir(data_folder)[i:i+5]]
      elif start_time_frame==None   and filename.endswith('.hdf'):
        fns=[f"{data_folder}/{file_name}" for file_name in os.listdir(data_folder)[1:5]]
...
def get_input_array(field,downscale256=True) -> np.ndarray:
    ...
    tensor = np.stack(preprocessed_data)
    return tensor
